...
class search_s():
    ...
    def __init__(self, Osn) -> None:
        ...
        Osn.add(self.page_base, text='  Разделение строки по словам  ')
        ...
        self.page_base_label_Nom2_Slovo = Label(self.page_base, text="Исходный текст")
        ...
        self.page_base_read_bt_pit.place(
            x=190, y=geom_par['heigh_y'] + 40, anchor="w", heigh=30, width=150,
            bordermode=OUTSIDE)

        self.kolSimb = Text(self.page_base)
        self.kolSimb.place(
            x=345, y=geom_par['heigh_y'] + 25, anchor="nw",
            heigh=30, width=50,
            bordermode=OUTSIDE)
        ...
        self.page_base_btn_clear.place(
            x=400, y=geom_par['heigh_y'] + 40, anchor="w", heigh=30,
            width=150, bordermode=OUTSIDE)

        self.page_base_lineText_DublSlovo = Text(self.page_base)  # all text
        self.page_base_lineText_DublSlovo.place(x=10, y=geom_par['heigh_y'] + 60,
                                            anchor="nw", heigh=500,
                                            width=geom_par['P2widthW'],
                                            bordermode=OUTSIDE)
        ...
        self.btn_token = Button(self.page_base, text="Токинезация", command=func4)
        ...
        self.btn_stat = Button(self.page_base, text="Статистика", command=func5)
        ...
        # self.page_base_btn_Kontext = Button(self.page_base, text="Разделить текст",
