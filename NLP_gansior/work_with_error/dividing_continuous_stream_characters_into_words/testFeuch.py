...
def statTextList(text_list: list) -> list:
    """_summary_

    Args:
        textList (list): _description_
    ...
    for k in stat.most_common():
        text_out.append([k[0], k[1]])
    return text_out


def baseConcat(dd: dict,
               reserch_text: dict,
               lengthFirstWord: int,
               ff2: int):
    """_summary_
    ...
    Args:
        ...
        reserch_text (list): _description_
    ...
    # print('key =', key)
    while indItem < len(reserch_text['CountText']):
        startLen = len(reserch_text['CountText'])
        if startLen > 1:
            match key:
                case 44:
                    if len(reserch_text['CountText'][indItem]) >= 4 and indItem < (len(reserch_text['CountText']) - 1):
                        if len(reserch_text['CountText'][indItem + 1]) >= 4:
                            frag = reserch_text['CountText'][indItem][-4:] + \
                                reserch_text['CountText'][indItem + 1][:4]
                            if frag in dd[key]:
                                dd[key][frag]['k_l'] += 1
                            else:
                                dd[key][frag] = {'txt': '', 'k_l': 1}
                                dd[key][frag]['txt'] = reserch_text['CountText'][indItem][-4:] + \
                                    ' ' + \
                                    reserch_text['CountText'][indItem + 1][:4]
                            reserch_text['CountText'][indItem] = reserch_text['CountText'][indItem] + \
                                reserch_text['CountText'][indItem + 1]
                            reserch_text['CountText'].pop(indItem + 1)
                case 14 | 24 | 34:
                    if len(reserch_text['CountText'][indItem]) == lengthFirstWord and indItem < (len(reserch_text['CountText']) - 1):
                        if len(reserch_text['CountText'][indItem + 1]) >= 4:
                            frag = reserch_text['CountText'][indItem] + \
                                reserch_text['CountText'][indItem + 1][:4]
                            if frag in dd[key]:
                                dd[key][frag]['k_l'] += 1
                            else:
                                dd[key][frag] = {'txt': '', 'k_l': 1}
                                dd[key][frag]['txt'] = (reserch_text['CountText'][indItem]
                                                        + ' ' +
                                                        reserch_text['CountText'][indItem + 1][:4])
                            reserch_text['CountText'][indItem] = (reserch_text['CountText'][indItem] +
                                                                  reserch_text['CountText'][indItem + 1])
                ...
                case 11 | 12 | 13 | 22 | 23 | 33:
                    # print('inn = ', key)
                    if (len(reserch_text['CountText'][indItem]) ==
                        lengthFirstWord and
                        indItem < (len(reserch_text['CountText']) - 1)
                        ):
                        if len(reserch_text['CountText'][indItem + 1]) == ff2:
                            frag = reserch_text['CountText'][indItem] + \
                                reserch_text['CountText'][indItem + 1]
                            if frag in dd[key]:
                                dd[key][frag]['k_l'] += 1
                                ...
                                dd[key][frag]['txt'] = reserch_text['CountText'][indItem] + \
                                    ' ' + reserch_text['CountText'][indItem + 1]
                            reserch_text['CountText'][indItem] = frag
                            reserch_text['CountText'].pop(indItem + 1)
            if startLen > len(reserch_text['CountText']):
                indItem += 0
            else:
                indItem += 1
        else:
            indItem = len(reserch_text['CountText'])+1
    reserch_text[key]['CountText'] = reserch_text['CountText'].copy()
    reserch_text[key]['stat'] = statTextList(reserch_text['CountText'])
    return dd, reserch_text
                ...
                if len(frr) > (firstStep + secondStep):
                    ...
                    if len(baseDict[indDict]) > 0:
                        while kk <= (len(frr)):
                            ...
                            if frag in baseDict[indDict]:
                                frrControl += frr[st:st +
                                                  firstStep + secondStep]
                                text_rez += ' ' + \
                                    baseDict[indDict][frag]['txt'] + ' '
...
if __name__ == '__main__':
    ...
    reserch_text_s = IshText.copy()
    dict_fr, rez_s, RezText = connectLearn(reserch_text_s)
    cprint(f'{RezText}', 'red', attrs=['bold'])
    cprint(f'{dict_fr}', 'blue', attrs=['bold'])
    DubleText = separatorWords(rez_s, dict_fr)
